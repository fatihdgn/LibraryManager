...
namespace Fthdgn.LibraryManager.UI.ViewModel
{
    ...
    public class LoanReturnViewModel : DetailedItemsViewModel<Loan, LoanViewModel, LoanReturnDetailViewModel>
    {
        public LoanReturnViewModel(ViewModelLocator locator, LoanReturnDetailViewModel detailViewModel, LibraryManagerManagers managers) : base(locator, detailViewModel, managers)
        {
            Name = nameof(LoanReturnViewModel);
            DisplayName = "Kitap İade Et";
            CreateText = "";
            CanCreate = false;
            CanSearch = true;
            CanSelect = true;
        ...
        protected override IEnumerable<Loan> ProvideItems() => Managers.Repositories.Loans.Query().Where(x => x.Library.Id == Locator.Main.Library.Id && x.User.Id == Locator.Main.User.Id && x.ReturnedAt == null).OrderBy(x => x.LoanedAt);
